...
3. 자동 표시된 IPv4가 수강생 기기에서 강사 PC의 공유 포트에 접속할 수 있고 학교·조직 정책이 허용하는 주소인지 확인합니다. Wi-Fi·이더넷·VPN 가운데 실제 수업 연결 경로에 맞는 IPv4를 입력하세요. VPN도 기기 간 TCP 연결과 정책 조건이 충족되면 사용할 수 있습니다.
4. **공유 시작**을 누릅니다. 사용 가능한 포트를 자동 선택하며 접속 주소와 QR이 표시됩니다.
5. 주소를 복사하거나 QR을 수강생에게 보여줍니다. 수강생은 강사 PC 주소에 접속할 수 있는 수업 네트워크에서 브라우저로 접속합니다. 별도 앱·계정은 필요 없습니다.
...
1. 학교·조직이 허용하며 강사 PC에 접속할 수 있는 수업 네트워크 또는 VPN에 연결합니다.
...
- 학교·게스트 Wi-Fi의 기기 간 격리, 서로 다른 VLAN의 라우팅·정책, VPN의 기기 간 연결 제한 또는 잘못된 IPv4는 접속을 막을 수 있습니다. 네트워크 관리자에게 **수강생 → 강사 PC 공유 포트**의 TCP 연결이 허용되는지 확인합니다. 강사 PC에서 다른 기기로 SSH 접속이 된다는 것만으로 반대 방향 HTTP 접속까지 확인되지는 않습니다.
...
2026-09-11에는 Windows의 실제 공유 서버와 Mac headless 브라우저 사이에서
SSH 중계 경로를 통해 파일 다운로드와 합성 프레임 갱신을 확인했습니다.
Mac에서 Windows VPN 주소로 직접 접속한 HTTP 요청은 시간 초과됐고 원인은
미확정입니다. 실제 모니터 캡처, 로그인한 Mac GUI, 수업 네트워크의 직접
연결 성공은 이 검사에 포함되지 않습니다. [검증 결과와 한계](../../docs/validation/0.2-windows-share-mac.ko.md)를 참고하세요.
