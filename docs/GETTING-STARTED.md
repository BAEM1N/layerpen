# Getting started with Pointory

Choose Settings → Language for English, 한국어, 日本語, or 简体中文. System language is selected automatically by default.

Pointory adds a drawing layer over one monitor. Use it while sharing your own screen or demonstrating a desktop application.
...
Close any older OnPen, LayerPen, or Monitor Ink instance before starting Pointory. Pointory imports legacy settings and fonts into its own configuration folder on first launch. A previous Monitor Ink installation may remain as a separate Windows installed-app entry; remove the old version through Windows Settings if you no longer need it.
...
1. Select your monitor in Pointory.
...
An application-window or browser-tab share may not contain the overlay. No meeting-service compatibility is claimed until tested from a participant's view. Pointory does not let other participants draw on your screen.
...
The default capture folder is Pictures/Pointory. Existing settings using Pictures/MonitorInk switch to the new folder; existing files stay in the old folder. Custom folders are preserved. Change the folder in Settings. The portable and installed editions share settings unless `POINTORY_DATA_DIR` points to a separate profile. `ONPEN_DATA_DIR` and `MONITOR_INK_DATA_DIR` remain accepted as compatibility aliases.

## Uninstall

Use Windows Settings → Apps → Pointory → Uninstall. Leave the installer’s app-data deletion option unchecked to preserve settings. Keep captures outside the installation and configuration directories. There is no automatic updater.
