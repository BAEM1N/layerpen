...
Pointory supports Korean (`ko`), English (`en`), Japanese (`ja`), and Simplified Chinese (`zh-CN`). Settings → Language updates all open application windows and persists the choice. System language is the default; unsupported languages fall back to English. Chinese regional variants currently use Simplified Chinese.
