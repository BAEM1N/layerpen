...
(async()=>{
 ...
 const run=async(name,fn)=>{await api.event.emit('pointory-validation-progress',name);try{const detail=await fn();report.checks.push({name,status:'passed',detail});}catch(e){report.checks.push({name,status:'failed',error:String(e)});}};
 const remote=(label,script)=>new Promise((resolve,reject)=>{
  ...
  api.event.emit('pointory-validation-eval',{id,label,script}).catch(reject);
 });
 try{
  ...
  await configure({language:'ko',layout:'horizontal',global_shortcut_enabled:false,capture_layer_only:true,gif_background:'white',gif_speed:4});
  await run('native-display-and-brand',async()=>{
   ...
   const brand=await remote('settings',"return {title:document.querySelector('h1')?.textContent,logo:document.querySelector('.logo img')?.naturalWidth};");
  ...
  await run('vertical-toolbar-and-width-preservation',async()=>{
   ...
   await call('action',{name:'toggle_visibility'});return {width:64,height:innerHeight};
  });
  await run('docked-settings-system-fonts-theme',async()=>{
   ...
   const family=fonts.includes('Apple SD Gothic Neo')?'Apple SD Gothic Neo':fonts[0];await configure({text_font:family,theme:'teal'});
   const info=await remote('settings',"return {directions:document.querySelectorAll('[data-layout]').length,lines:document.querySelectorAll('[data-lines]').length,selectedFont:document.querySelector('#textFont')?.value};");
   assert(info.directions===2&&info.lines===0,'Obsolete layout controls');assert((await call('snapshot')).preferences.text_font===family,'Font not saved');return {fontCount:fonts.length,family,info};
  });
  await run('settings-font-size-and-persistence',async()=>{
   ...
   const info=await remote('settings',"await document.fonts.ready;const body=getComputedStyle(document.body),select=getComputedStyle(document.querySelector('#settingsFontSize'));return {family:body.fontFamily,size:body.fontSize,controlSize:select.fontSize,scrollWidth:document.documentElement.scrollWidth,width:innerWidth};");
   ...
   await call('action',{name:'settings'});await call('action',{name:'settings'});await sleep(250);
  ...
  await run('native-webview-korean-text-entry',async()=>{
   ...
   await wait(async()=>(await call('snapshot')).strokes.some(stroke=>stroke.text?.content==='Pointory 맥 필기 테스트'||stroke.text==='Pointory 맥 필기 테스트'||stroke.content==='Pointory 맥 필기 테스트'),'Text stroke was not committed');return text;
  });
  await run('ink-undo-redo-and-png',async()=>{
   ...
   let path;const unlisten=await api.event.listen('capture-saved',event=>{path=event.payload;});await call('request_capture');await wait(()=>Boolean(path),'PNG export did not finish');unlisten();return {path};
  });
  ...
  await run('spotlight-toggle',async()=>{await call('spotlight_toggle');const enabled=await call('spotlight_status');await call('spotlight_toggle');return {enabled,disabled:await call('spotlight_status'),magnification:'not supported on macOS'};});
  await run('classroom-server-start-stop',async()=>{await call('share_start',{address:'127.0.0.1'});const running=await call('share_status');assert(running.running&&running.qr,'Server did not start');await call('share_stop');const stopped=await call('share_status');assert(!stopped.running,'Server did not stop');return {running,stopped};});
